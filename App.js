...
Ext.define('nantonelliTimeLineApp', {
        ...
        var itemStore = Ext.create('Rally.data.wsapi.Store', {
            ...
            listeners: {
                load: function(store, data, success) {
                    ...
                    var pb = Ext.getCmp('parentBox');
                    if (pb) {
                            var uniqs = _.uniq(_.map(data, function(record) { return record.get('Parent') ? '<a href="https://rally1.rallydev.com/#/detail' + record.get('Parent')._ref + '" target = "_top">' + record.get('Parent').FormattedID + '</a>: ' + record.get('Parent')._refObjectName : "Unknown";}));
                            _.each (uniqs, function(parent) {
                                var found = _.filter(data, function(record) {
                                        return (record.get('Parent') ? '<a href="https://rally1.rallydev.com/#/detail' + record.get('Parent')._ref + '" target = "_top">' + record.get('Parent').FormattedID + '</a>: ' + record.get('Parent')._refObjectName : "Unknown") === parent;
                                });
                                var parentBox = {};
    ...
    _createTitleBoxForItem: function(app, item) {
        ...
        titleRec.width = '100%';
        titleRec.title = '<a href="https://rally1.rallydev.com/#/detail' + item.get('_ref') + '" target = "_top">' + item.get('FormattedID') + '</a>: ' + item.get('Name');
